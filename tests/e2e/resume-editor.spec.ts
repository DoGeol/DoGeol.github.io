...
test('대형 기술 편집 UI를 열 때만 마운트하고 filter가 선택 순서를 바꾸지 않는다', async ({
  page,
}) => {
  await waitForEditor(page)

  await expect(page.getByLabel('기술 ID')).toHaveCount(0)
  await page.getByRole('button', { name: '기술 목록 57개 열기' }).click()
  await expect(page.getByLabel('기술 ID')).toHaveCount(57)
  await page.getByRole('button', { name: '기술 목록 닫기' }).click()
  await expect(page.getByLabel('기술 ID')).toHaveCount(0)

  await openSection(page, '경력')
  const skillField = page.getByRole('group', { name: '사용 기술' }).first()
  const selectedHandles = skillField.locator('button[aria-roledescription="sortable"]')
  const beforeOrder = await selectedHandles.evaluateAll((elements) =>
    elements.map((element) => element.getAttribute('aria-label')),
  )
  await expect(skillField.getByRole('checkbox')).toHaveCount(0)
  await skillField.getByRole('button', { name: '기술 선택기 열기' }).click()
  await skillField.getByRole('searchbox', { name: '기술 검색' }).fill('react')
  await skillField.getByRole('combobox', { name: '기술 분류 필터' }).selectOption('frontend')
  await expect(skillField.getByRole('checkbox', { name: /React/i })).toHaveCount(3)
  expect(
    await selectedHandles.evaluateAll((elements) =>
      elements.map((element) => element.getAttribute('aria-label')),
    ),
  ).toEqual(beforeOrder)
  expect(
