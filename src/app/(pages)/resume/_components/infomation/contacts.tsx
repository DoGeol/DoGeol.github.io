...
import type { InformationSection } from '@/app/(pages)/resume/_model/resume-schema'
import type { ResumeRegionRenderer } from '@/app/(pages)/resume/_model/resume-region'
import { cn } from '@/shared/lib/tailwindcss'

type Contact = InformationSection['data']['contacts'][number]

interface Props {
  contactList: InformationSection['data']['contacts']
  renderRegion: ResumeRegionRenderer
}

const RenderIcon = (type: Contact['type']) => {
  if (type === 'github') {
    return (
      <svg
...
const getContactHref = ({ type, value }: Contact) => {
  if (type === 'email') return `mailto:${value}`
  if (type === 'tel') return `tel:${value}`
  return value
}

const Contacts = ({ contactList, renderRegion }: Props) => (
  <ul
    className={cn(
        ...
        {renderRegion({
          ...
          children: (
            ...
            >
              ...
              >
                <span className="inline-flex size-6 items-center justify-center text-neutral-900 dark:text-neutral-400">
