...
`resume-editor.spec.ts`는 edit→preview 선택, 표시 toggle, keyboard reorder, company CRUD, session restore/reset, validation focus, JSON download, actual mode, 잘못된 asset과 public 격리를 독립 test로 검증한다. build 전 asset script는 canonical image를, build 후 static-export script는 dev route와 marker 부재를 확인한다.
